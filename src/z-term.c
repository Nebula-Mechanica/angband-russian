...
static errr term_win_nuke(term_win *s)
{
	...
	/* Free the graphics flags arrays */
	KILL(s->g);
	KILL(s->vg);
...
static errr term_win_init(term_win *s, int w, int h)
{
	...
	/* Make the graphics flags arrays */
	C_MAKE(s->g, h, byte*);
	C_MAKE(s->vg, h * w, byte);
...
void Term_queue_char(int x, int y, byte a, char c, byte ta, char tc)
{
	...
	byte *scr_g = Term->scr->g[y];
	byte og = scr_g[x];
	
	byte g = (byte)Term_graphics;
	...
	if ((oa == a) && (oc == c) && (ota == ta) && (otc == tc) && (og == g)) return;
...
void Term_queue_chars(int x, int y, int n, byte a, cptr s)
{
	...
	for ( ; n; x++, s++, n--)
	{
		...
		/* Hack -- Ignore non-changes */
		if ((oa == a) && (oc == *s) && (ota == 0) && (otc == 0) && (og == g)) continue;
